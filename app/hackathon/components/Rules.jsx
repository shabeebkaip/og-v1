...
const Rules = ({ hackathonData, timeZones }) => {
  const [remainingDays, setRemainingDays] = useState(null);
  const [timeZone, setTimeZone] = useState("");
  const [convertedTime, setConvertedTime] = useState("");
  const [open, setOpen] = useState(false);
 console.log('====================================');
 console.log(timeZone,"hhhh");
 console.log('====================================');
  ...
  const handleTimeZoneChange = (time, fromTimeZone, toTimeZone) => {
    const data = moment
      .tz(time, fromTimeZone)
      .tz(toTimeZone)
      .format("YYYY-MM-DD HH:mm:ss");
    console.log(data);
    setConvertedTime(data);
    return data;
  };
  const handleSelectChange = (event, newValue) => {
    if (newValue) {
        setTimeZone(newValue);
        if (hackathonData?.time && hackathonData?.timezone) {
            handleTimeZoneChange(hackathonData.time, hackathonData.timezone, newValue.value);
        }
    } else {
        setTimeZone(null);
    }
  ...
  return (
    <div className="flex flex-col justify-between gap-7 md:flex-row mt-16 md:p-3">
      <div className="md:w-[45%] flex flex-col justify-center gap-12">
        <div>
          <h1 className="xl:text-[50px]  lg:text-[40px] text-[30px]  font-medium text-[#FF8500] uppercase">
            {hackathonData?.ruleHeading}
          </h1>
          <p className="xl:text-[50px]  lg:text-[40px] text-[30px] font-medium text-[#4C4C4D]">
            {hackathonData?.ruleDescription}
          </p>
        </div>
        <div className="flex justify-between">
          ...
          <p
            className="text-[#FF8500] text-[16px] md:hidden block "
        ...
        <div className="lg:flex flex-col gap-5 hidden">
          <ul className="flex gap-10 text-[16px] text-[#1C2126] list-disc  flex-col md:grid grid-cols-3 ml-4">
            {hackathonData?.who_can_participate
              ?.slice(0, 6)
              .map((item, index) => (
        ...
        <div className="hidden md:block">
          <p
            className="text-[#FF8500] text-[16px] cursor-pointer"
            style={{ textDecoration: "underline" }}
            onClick={handleModalItem}
          >
            View full rules
        ...
        <div className="box-shadow rounded-[23px] flex flex-col justify-center md:p-5">
          {remainingDays > 0 && (
            <div className="flex flex-wrap gap-4 p-5 ">
              <button className="border border-[#92D1FB] rounded-[40px] text-[16px] 2xl:p-3 p-2 hover:text-white hover:bg-[#FF8500] px-5">
                {remainingDays} more days to deadline
              </button>
            </div>
          )}
            ...
            <div className="flex  gap-2 justify-cente items-center">
              <p>
                {hackathonData?.end_date
                  ? moment(hackathonData?.end_date).format(
          ...
          <div className="flex gap-10 p-5">
            <Image
              src="/hackathone/flag.png"
              className="w-6 h-6"
              width={200}
              height={200}
              alt=""
            ></Image>
            <p className="text-[16px] border border-[#4C4C4D] rounded-[40px] px-5">
              {hackathonData?.host}
            </p>
          </div>
          <div className="flex gap-2 p-5">
            ...
            >
              {hackathonData?.tags?.map((item, index) => (
                <button
                  key={index}
                  className="border border-[#92D1FB] rounded-[40px] text-xs md:text-[16px] p-2"
                >
                  {item.text}
                </button>
              ))}
          ...
          <div className="flex items-center pb-12">
            <Image
              src={hackathonData?.managedByimage}
              className="w-[100px]"
              alt=""
              width={200}
              height={200}
            />
            <p className="text-[16px] text-[#1C2126] font-normal">
              Managed by {hackathonData?.managedByname}
            </p>
          </div>
        </div>
      </div>
