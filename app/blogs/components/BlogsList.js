      ...
      {loading ? (
        ...
        <div className="flex flex-col">
          <div className="relative flex flex-row justify-center gap-4 px-10 md:justify-end capitalize">
            <p className={`text-base font-medium cursor-pointer ${selectedCategoryId === null && 'bg-[#FF8500]  text-white px-3 rounded-xl'}`} onClick={() => handleCategory(null)}>
              all
            </p>
          ...
          <div className="">
            {noNewsFound ? (
              <p className=" mt-8 flex justify-around text-red-500 text-[20px]">No News Found!</p>
            ) : (
              <div className="flex-col p-2 md:grid md:grid-cols-4 2xl:gap-2 lg:gap-3 lg:p-5 md:p-2 md:gap-2">
                {newsList.slice(0, visible).map((item, index) => (
          ...
          {!noNewsFound && (
            <div className="flex flex-row items-center justify-center pt-8">
              {visible < newsList.length && (
                <button onClick={loadMore} className="text-[26px] font-light leading-[30.44px] flex gap-5 lg:text-[25px] 2xl:text-[30px] md:text-[20px]">
                  Load More <span className="text-[#FF8500] w-[27.22px] h-[30.01px] md:mt-1"><SlReload /> </span>
                </button>
              )}
            </div>
          )}
