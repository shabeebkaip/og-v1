        ...
        <SnackbarProvider>
            <div className="container mx-auto">
                <div className="md:grid flex flex-col p-2  md:grid-cols-4 2xl:gap-2 lg:gap-3 lg:p-5 md:p-2 md:gap-2">
                    {courseList?.slice(0, visible).map((item) => (
                        <div key={item?.mode} className="p-1  flex flex-col justify-between ">
                            <div className="mt-6 ">
                                ...
                                <p className="text-[22px] font-light leading-[25.81px] lg:text-[19px] lg:leading-tight 2xl:text-[22px] md:text-[15px] md:leading-tight">{item?.description}</p>
                            </div>
                            <div>
                                <button onClick={() => {
                                    setPopup(true)
                                    setCourseName(item?.heading)
                                }} className="mt-6  rounded-full w-[165px] h-[39px] md:w-[100px] md:text-[10px] md:h-[28px] lg:w-[165px] lg:h-[39px] lg:text-[12px]" style={{ backgroundColor: item?.mode === 'Online' ? '#92D1FB' : item?.mode === 'Hybrid' ? '#FF8500' : item?.mode === 'Offline' ? 'white' : 'none', border: item?.mode === 'Offline' ? '1px solid orange' : 'none', color: item?.mode === 'Offline' ? '#4C4C4D' : 'white' }}>Apply now<span className="ml-3">&rarr;</span></button>
                                <Link href={`/educations/${item._id}`}><button className="px-2 text-orange-500 underline" >View</button></Link>
                            </div>
