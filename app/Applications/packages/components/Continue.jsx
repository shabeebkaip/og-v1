        ...
        if (checked) {
            ...
            axios.post('https://api-one-global.code-ox.com/api/save-order', data, {
                ...
                .then(response => {
                    ...
                    window.location.href = `/payment?ref=${referenceID}&id=${id}`
                })
        } else {
    ...
    return (
        <div className='relative pb-8'>
            ...
            <div className='flex items-center justify-center my-20 cursor-pointer' onClick={handleInitiatePayment}>
                <p className={`border  rounded-[40px] ${!checked ? 'border-gray-500 text-gray-200 hover:border-gray-400 hover:text-gray-200' : 'hover:bg-[#FF8500]'} border-[#FF8500]  hover:text-white ${checked} text-[20px] text-[#FF8500] px-16 py-2`}>
                    Continue
                </p>
            </div>
