...
import { displayDateFormatShort } from "../../../../constant";
import Image from 'next/image';


const NewsDetailHero = ({newsDetail}) => {

  const midpoint = Math.ceil(newsDetail?.description?.length / 2);
  ...
  const renderDescriptionWithImages = (descriptionArray, startIndex) => {
    return descriptionArray?.map((item, index) => {
      ...
      if (newsDetail?.image?.[imageIndex] && imageIndex % 2 == 0) {
        return (
          <div key={index}>
            <Image width={1000} height={500} className="xl:h-[370px] h-[250px] object-cover rounded-3xl" src={newsDetail.image[imageIndex]} alt={newsDetail.image[imageIndex].alt}  />
    ...
    <div className='flex flex-col gap-10 relative '>
      ...
      <div className='container mx-auto '>
        <div className="md:text-5xl text-3xl relative sm:px-0 px-3 font-medium text-[#656565] text-center py-16">
          <p className='capitalize'>{newsDetail?.title}</p>
          ...
          <div className="bg-white rounded-3xl box-shadow">
            <div className="m-5 rounded-[10px]">
              <h2 className="md:text-3xl text-xl font-normal mb-4 leading-23 tracking-normal  ">
                <span className="block capitalize ">{newsDetail?.shortDescription}</span>
              </h2>
              <div className="flex gap-5 ">
                <p className=' capitalize'>{newsDetail?.createdAt ? moment(newsDetail?.createdAt).format(displayDateFormatShort) : "--"}</p>
                <p>{new Date(newsDetail?.createdAt).toLocaleTimeString('en-US', {
                  hour: 'numeric',
                  minute: 'numeric',
                  hour12: true,
                })}</p>
              ...
              <div className="flex gap-8 flex-col md:flex-row mt-12 ">
                <div className="w-full  md:w-1/2 flex flex-col gap-8 xl:text-[24px] text-lg text-[#4C4C4D] font-light capitalize">
                  {renderDescriptionWithImages(newsDetail?.description?.slice( 0,midpoint), 0)}
                </div>
                <div className="flex flex-col gap-8 md:w-1/2 xl:text-[24px] text-lg text-[#4C4C4D] w-full font-light capitalize">
                  {renderDescriptionWithImagesRight(newsDetail?.description?.slice(midpoint), 0)}
                </div>
              </div>
            </div>
          </div>
