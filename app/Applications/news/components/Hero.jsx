import BlueGradientRight from "../../../shared/components/BlueGradientRight";
...
const Hero = async () => {
  const heroResponse = await axios.get(`${baseURL}/hero?key=news`)
  const hero = heroResponse.data

  return (
    <div>
      <div className=" bg-[#EFEFEF] relative  lg:rounded-none rounded-3xl md:p-14 lg:m-0 sm:m-4 m-3 drop-shadow-xl lg:drop-shadow-none ">
          ...
          <div className="flex flex-col gap-6 justify-center h-[550px] lg:text-4xl font-medium text-3xl text-center items-center ">
            <div className="flex text-[#6D6E71] px-0  lg:px-24 lg:leading-[58px]   ">
              ...
              <div className="flex flex-col justify-center items-center p-6 h-full font-normal tracking-wide bg-[#fffff]  rounded-3xl text-3xl text-[#6D6E71] leading-32">
                <h1 className="text-[50px] font-medium text-center leading-[60px]">
                  <span>
                    {hero?.text?.split(hero?.borderText).map((splitText, index) => (
                      <div key={index} style={{ display: 'inline' }}>
                        {index > 0 && (
                          <span
                            className="py-2 px-5 border-2 border-[#6D6E71]  rounded-[53px]"
                            style={{
                ...
                <h1 className="text-[#FF8500] py-6 text-[50px]">{hero.text_1}</h1>
              </div>
            </div>
          </div>
        </div>
